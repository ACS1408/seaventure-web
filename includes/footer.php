<div class="footer">
    <div class="footer-top">
        <div class="container">
            <div class="footer-top__wrap">
                <div class="logo-col">
                    <figure class="footer-logo">
                        <img src="assets/images/seaventure-logo-lg.svg" alt="seaventure-logo">
                    </figure>
                </div>
                <nav class="links-col">
                    <dl class="page-links">
                        <dt class="active">Quick Links</dd>
                        <dd>
                            <a href="">Home</a>
                        </dd>
                        <dd>
                            <a href="#" data-section="about">About</a>
                        </dd>
                        <dd>
                            <a href="#" data-section="services">Services</a>
                        </dd>
                        <dd>
                            <a href="#" data-section="contact">Contact</a>
                        </dd>
                    </dl>
                    <dl class="page-links contact-info">
                        <dt>Contact Info</dt>
                        <dd>
                            <i class="icon-phone" style="font-size: 18px;"></i>
                            <a href="tel:+97143887184">+971 4 388 7184</a>
                        </dd>
                        <dd>
                            <i class="icon-envelope" style="font-size: 18px;"></i> 
                            <ul>
                                <li>
                                    <a href="mailto:contact@seaventure.live">contact@seaventure.live</a>
                                </li>
                                <li>
                                    <a href="mailto:manager@seaventure.live">manager@seaventure.live</a>
                                </li>
                            </ul>
                        </dd>
                    </dl>
                    <dl class="page-links">
                        <dt>Company Address</dt>
                        <dd>
                            <address>Seaventure Shipping Services,</address>
                            <address>226, Anchorage Building,</address>
                            <address>Dubai South, United Arab Emirates</address>
                        </dd>
                    </dl>
                    <dl class="page-links">
                        <dt>Connect</dt>
                        <dd>
                            <a href="http://www.linkedin.com/company/seaventure" target="_blank" rel="noopener noreferrer">LinkedIn</a>
                        </dd>
                        <dd>
                            <a href="http://www.facebook.com/seaventure.live" target="_blank" rel="noopener noreferrer">Facebook</a>
                        </dd>
                        <dd>
                            <a href="https://wa.me/97143887184?text=Test" target="_blank" rel="noopener noreferrer">Whatsapp</a>
                        </dd>
                        <dd>
                            <a href="https://instagram.com/teamseaventure?igshid=YmMyMTA2M2Y=" target="_blank" rel="noopener noreferrer">Instagram</a>
                        </dd>
                     
                       
                    </dl>
                </nav>
            </div>

        </div>
    </div>
    <div class="footer-bottom">
        <div class="container">
            <div class="copyrights">
                Copyrights @<?php echo date("Y");?> Seaventure. All rights reserved
            </div>
            <ul class="policy-links">
                <li>
                    <a href="privacy-policy.php">Privacy Policy</a>
                </li>
                <li>
                    <a href="terms-and-condition.php">Terms and Conditions</a>
                </li>
            </ul>
            <!-- <ul class="socials">
                <li>
                    <a href="http://" target="_blank" rel="noopener noreferrer">Whatsapp</a>
                </li>
                <li>
                    <a href="http://" target="_blank" rel="noopener noreferrer">Instagram</a>
                </li>
                <li>
                    <a href="http://" target="_blank" rel="noopener noreferrer">Twitter</a>
                </li>
            </ul> -->
        </div>
    </div>
</div>